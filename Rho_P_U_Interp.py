...
# ------------------------ # Interpolating rho, P, U # ------------------------ #
rho_new=10**np.linspace(np.log10(rho_input[0]),np.log10(rho_input[-1]),n) # Rho for S0=1100 data 
yrP = interp1d(rho_input,P_input); P_new = yrP(rho_new)
yPU = interp1d(P_input,U_input); U_new = yPU(P_new)

rho_new2=10**np.linspace(np.log10(rho_input2[0]),np.log10(rho_input2[-1]),n) # Rho for S0=3160 data
...
# ------------------------ # Adjusting U from 88K-1187K to 300K-1700K # ------------------------ #
U_target = np.linspace(300,1700,11)*1000
...
# ------------------------ # Saving to rho_u_S9999.dat file # ------------------------ #
