...
n=50 # of points for interpolated data
...
# ------------------------ # Adjusting U from 88K-1187K to 307K-1946K # ------------------------ #
U_offset = U_all[2,:] - U_all[0,:] # How much to raise floor by
U_diff = U_all[1,:] - U_all[0,:] # step size
U_fix = U_all + U_offset 
for i in np.arange(len(U_all)):
    U_fix[i,:] += U_diff *((2e6 - U_all[-1,0] - U_offset[0])/U_diff[0]) * i/11

rho_u_data=np.append(rho_new,P_new/1e9)
rho_u_data=np.append(rho_u_data,U_fix)
